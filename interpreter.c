...
foundparen(char *funstr, int llen, int lnum)
{
	...
	for (i = 0; i < llen; ++i, p++) {
		if (pctr == 0 && *p != ' ' && *p != '\t' && *p != '(') {
			dobeep();
			if (*p == ')')
				ewprintf("Extra ')' found on line: %d",
					 lnm);
			else
		...
		if (*p == '\\') {
			...
			if (lastchr != NULL && *lastchr == '(') {
				dobeep();
				ewprintf("Multiple consecutive left"
					 " parantheses line %d", lnm);
				return (FALSE);
			}
			if (inquote == 0) {
				if (begp != NULL) {
				...
				cleanup();
				dobeep();
				ewprintf("Opening and closing quote"
					 " char error line: %d", lnm);
				return (FALSE);
			}
			esc = spc = 0;
		} else if (*p == ')') {
			if (lastchr != NULL && *lastchr == '(') {
				dobeep();
				ewprintf("Empty parenthesis not"
					 " supported line %d", lnm);
				return (FALSE);
			}
			if (inquote == 0) {
				if (begp != NULL) {
	...
	if (pctr != 0) {
		cleanup();
		dobeep();
		ewprintf("Opening and closing parentheses error line:"
			 " %d", lnm);
		return (FALSE);
	}
	if (ret == FALSE)
